//!
//! This example showcases the Microsoft OAuth2 process using passport for requesting access to the the user's profile.
//!
...
pub async fn msft(passport: Data<RwLock<Passport>>) -> HttpResponse {
    let mut auth = passport.write().await;
    auth.authenticate(Choice::Microsoft).unwrap();
